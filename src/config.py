...
# ===== 付费推广 APP 过滤 =====
# 国内商店(华为/小米/vivo)搜索结果常混入与关键词无关的付费推广位 APP，
# 会虚高「同类数」把蓝海误判成红海。两层过滤：
# 1) 静态黑名单：名字命中即剔除（子串匹配，大小写不敏感）
# 2) 跨查询去重：同一 APP 名在一次运行中出现在 >= PROMOTION_REPEAT_THRESHOLD 个不同查询词里，
#    判定为推广位并剔除（推广位常跨多个不相关关键词反复展示）
PROMOTION_BLACKLIST = [
    # 电商 / 团购（常在技术词搜索里买推广位）
    "拼多多", "淘宝", "京东", "天猫", "苏宁", "唯品会", "快手电商",
    # 短视频 / 直播
    "抖音", "快手", "火山小视频", "西瓜视频", "好看视频",
    # 社交 / 浏览器 / 输入法推广位常客
    "微信", "QQ浏览器", "UC浏览器", "百度", "搜狗输入法", "讯飞输入法",
    # 游戏 / 娱乐推广位
    "原神", "王者荣耀", "和平精英", "蛋仔派对",
]
# 跨查询重复阈值：同一 APP 名出现在 >= N 个不同查询词结果里即视为推广
PROMOTION_REPEAT_THRESHOLD = 3
# 相关性兜底：国内商店结果中，APP 名与查询词字符交集比例 < 此值视为可疑推广
# （设 0.0 表示不启用相关性兜底，仅靠黑名单+跨查询去重）
PROMOTION_RELEVANCE_MIN = 0.15
